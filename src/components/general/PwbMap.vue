<template>
  <gmap-map ref="gmapRef" style="min-height: 600px;" :zoom="15" :center="center">
    ...
    <gmap-marker v-for="(m,i) in mapMarkers" :key="m.id" :position="m.position" @mouseover="toggleInfoWindow(m,i)" @mouseout="statusText = null" :clickable="true" :draggable="false" @click="toggleInfoWindow(m,i)"></gmap-marker>
  </gmap-map>
</template>
<script>
export default {
  ...
  methods: {
    ...
    toggleInfoWindow: function(marker, index) {
      ...
      var infoWindowContent = '<div class="iw-container">' +
        ...
        image_html +
        '</div>' +
        '<div class="iw-title primary--text">' + marker.title + '</div>' +
        '</a>' +
        '</div>'
